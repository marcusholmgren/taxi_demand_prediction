...
def download_one_file_of_raw_data(year: int, month: int) -> Path:
    ...
    if response.status_code == 200:
        path = Path(RAW_DATA_DIR.joinpath(f'rides_{year}-{month:02d}.parquet'))
...
def validate_raw_data(
    ...
    rides = rides[rides['pickup_datetime'].between(this_month_start, next_month_start)]
...
def load_raw_data(
    ...
    for month in months:
        local_file = Path(RAW_DATA_DIR.joinpath(f'rides_{year}-{month:02d}.parquet'))
